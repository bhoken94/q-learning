...
class RepeatActionAndMaxFrame(gym.Wrapper):
    ...
    def step(self, action):
        ...
        return max_frame, tot_rewards, done, info
...
class PreprocessFrame(gym.ObservationWrapper):
    ...
    def observation(self, observation):
        new_frame = cv2.cvtColor(observation, cv2.COLOR_RGB2GRAY) # converto il frame in scala di grigi
...
def make_env(env_name, shape=(84,84,1), repeat=4, clip_rewards=False, no_ops=0, fire_first=False):
    ...
    env = RepeatActionAndMaxFrame(env=env, repeat=repeat, clip_reward=clip_rewards, no_ops=no_ops, fire_first=fire_first)
