...
class Agent:
    ...
    def choose_action(self, observation):
        if np.random.random() > self.epsilon:
            ...
            action = T.argmax(actions).item()  # .item() serve per prendere il valore all'interno del tensor ritornato
        else:
    ...
    def learn(self, state, action, reward, state_):
        ...
        q_values = self.Q.forward(states)[actions]
        q_next = self.Q.forward(states_).max()
        q_target = rewards + self.gamma * q_next

        loss = self.Q.loss(q_target, q_values).to(self.Q.device)
        loss.backward()
...
if __name__ == "__main__":
    ...
    for i in range(num_games):
        ...
        if i % 100 == 0:
            ...
            print("=> Episode {} score {} avg score {} epsilon {}".format(i, score, avg_score, agent.epsilon))
    filename = "../plots/cartpole_naive_dqn.png"
    x = [n+1 for n in range(num_games)]
